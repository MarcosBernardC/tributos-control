(function() {
    ...
    // Variables privadas (no accesibles desde consola F12)
    let sessionToken = null;
    let isPrivateMode = true; // Por defecto empezamos en modo privado
    let currentData = null; // Guardará la info actual para redibujar sin refetch
        ...
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const response = await fetch(`${API_URL}/login`, {
                    ...
                    body: JSON.stringify({ ruc: rucValue, password: passValue })
                });
    ...
    async function loadData() {
        if (!sessionToken) {
            showToast('Sesión no válida. Por favor, ingrese de nuevo.', 'error');
            return;
        }
    ...
    function renderPropietario(propietario) {
        ...
        if (isPrivateMode && rucVisible && rucVisible.length > 1) {
            ...
            card.innerHTML = `
                ...
                <div class="inquilino-details">
                    <div class="detail-row">
                        ...
                        <span class="value">${isPrivateMode ? '•••• Oculto' : inq.dni}</span>
                    </div>
                    <div class="detail-row">
